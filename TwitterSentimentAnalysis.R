...
test_pred=ifelse(y_pred>0.5, 1,0)
submission$label = test_pred
...
set.seed(1234)
my_control=trainControl(method="cv",number =5)
tgrid = expand.grid(.mtry=seq(5,30,by=5))
classifier = train(x=training_set[,-(which(colnames(training_set)=="label"))],
                     y=training_set$label, method="rf",
                     trControl = my_control, tuneGrid = tgrid,
                     num.trees=300,  oob_score=TRUE, splitrule="gini", min_node_size=20)
plot(classifier) # We can see best score at mtry =  
y_pred = predict(classifier, newdata = test_set[,-1108])
test_pred=ifelse(y_pred>0.5, 1,0)
submission$label = test_pred
...
#XGBoost
library(xgboost)
training_set$label = as.numeric(training_set$label)
training_set$label = training_set$label-1
param_list = list(objective="binary:logistic", eta=0.1,max_depth=8,subsample=0.9,colsample_bytree=0.5)
dtrain=xgb.DMatrix(data = as.matrix(training_set[,-(which(colnames(training_set)=="label"))]), 
                   label= training_set$label) 
dtest = xgb.DMatrix(data = as.matrix(test_set[,-(which(colnames(training_set)=="label"))]))
#Applying cross validation to find optimal values for nrounds
set.seed(1234)
xgbcv=xgb.cv(params = param_list, data=dtrain, nrounds=1500, nfold=5, metric = list("rmse","auc"),
             print_every_n = 10, early_stopping_rounds = 30, maximize = T)
#Got best result at 406
xgb_regressor = xgb.train(data=dtrain, params=param_list,nrounds = 406)
#Predicting target variable
y_pred = predict(xgb_regressor, dtest)
test_pred=ifelse(y_pred>0.5, 1,0)
submission$label = test_pred
write.csv(submission, "XGBoost_Result.csv", row.names =FALSE)
#Variable Importance
var_imp = xgb.importance(feature_names = setdiff(names(training_set),c("label")),
                         model=xgb_regressor)
